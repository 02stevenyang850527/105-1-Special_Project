function [mask, face_location] = mySkinMask(im_recover)
[n,m] = size(im_recover);
area = n*m;
...
%         poss_x = find(pic_position(:,1) > n);
%         if (~isempty(poss_x))
%             pic_position(poss_x) = pic_position(poss_x)-1;
%         end;
%         poss_y = find(pic_position(:,2) > m);
%         if (~isempty(poss_y))
%             pic_position(poss_y) = pic_position(poss_y)-1;
%         end;  
        idx = mysub2ind(size(mask),pic_position(:,1),pic_position(:,2));
        mask(idx) = 255;
